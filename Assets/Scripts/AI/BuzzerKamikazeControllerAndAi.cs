...
public class BuzzerKamikazeControllerAndAi : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (rechargeTimer < 0.0f && threatRange && Vector3.Dot(character.forward, direction) > 0.8f)
        {
            ...
            if (targetHealth)
            {
                var playerDir = player.position - character.position;
    ...
    private System.Collections.IEnumerator DoElectricArc()
    {
        ...
        audioSource.Play();
        //buzz.didChargeEffect = false;//TODO
