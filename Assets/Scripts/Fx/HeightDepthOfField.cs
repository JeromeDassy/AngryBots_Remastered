...
public class HeightDepthOfField : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        dofMaterial.SetVector("_ObjectFocusParameter", new Vector4(
            t.position.y - 0.25f,
            t.localScale.y * 1.0f / smoothness,
            1.0f,
    ...
    void Blur(RenderTexture from, RenderTexture to, int iterations, int blurPass, float spread)
    {
        ...
        else
        {
            ...
            var passOffs = blurPass;
            for (var i = 0; i < iterations; i++)
            {
                dofBlurMaterial.SetVector("offsets", new Vector4(spread * oneOverBaseSize, 0.0f, 0.0f, 0.0f));
                to.DiscardContents();
                Graphics.Blit(tmp, to, dofBlurMaterial, passOffs);
                dofBlurMaterial.SetVector("offsets", new Vector4(0.0f, spread * oneOverBaseSize, 0.0f, 0.0f));
                tmp.DiscardContents();
                Graphics.Blit(to, tmp, dofBlurMaterial, passOffs + 1);
                passOffs += 2;
            }
        }
